...
const TABLE_HEAD = [
  { id: 'category', label: 'Category', width: "30%", search: true , sort: true },
  { id: 'make', label: 'Make', width: "30%", search: true , sort: true },
  { id: 'status', label: 'Status', width: "15%", search: true , sort: true },
  { id: 'created', label: 'Created', width: "15%", search: true , sort: true, order: false },
...
const AdminMakes = () => {
  ...
  const handleDeleteRecord = () => {
    APIHandler.adminDeleteMake(`(${rowData.id})`).then(data => {
      if (data.result === 'true') {
  ...
  const handleActivateRecords = (status) => {
    ...
    APIHandler.adminActiveMake(`(${ids})`, status).then(data => {
      if (data.result === 'true') {
  ...
  const handleDeleteRecords = () => {
    const ids = getIdString(selected);
    APIHandler.adminDeleteMake(`(${ids})`).then(data => {
      if (data.result === 'true') {
  ...
  return (
    <div className="container-fluid px-4 py-4">
      <div className="d-flex align-items-center justify-content-between">
        <h2 className="fw-600 fs-1p25 midnight">Make Management</h2>
