...
const TABLE_HEAD = [
  { id: 'category', label: 'Category', width: "10%", search: true , sort: true },
  { id: 'make', label: 'Make', width: "25%", search: true , sort: true },
  { id: 'model', label: 'Model', width: "20%", search: true , sort: true },
  { id: 'startyear', label: 'Start Year', width: "10%", search: true , sort: true },
  { id: 'endyear', label: 'End Year', width: "10%", search: true , sort: true },
...
const AdminModels = () => {
  ...
  useEffect(() => {
    ...
    APIHandler.adminAllMakes().then(data => {
      if (isMounted) {
        setMakes(data.filter(e => e.status === 1));
      }
    });
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    APIHandler.adminAllModels().then(data => {
      if (isMounted) {
        setAllData(data);
        setLoading(false);
      }
  ...
  const handleDeleteRecord = () => {
    APIHandler.adminDeleteModel(`(${rowData.id})`).then(data => {
      if (data.result === 'true') {
  ...
  const handleActivateRecords = (status) => {
    ...
    APIHandler.adminActiveModel(`(${ids})`, status).then(data => {
      if (data.result === 'true') {
  ...
  const handleDeleteRecords = () => {
    const ids = getIdString(selected);
    APIHandler.adminDeleteModel(`(${ids})`).then(data => {
      if (data.result === 'true') {
  ...
  return (
    <div className="container-fluid px-4 py-4">
      <div className="d-flex align-items-center justify-content-between">
        <h2 className="fw-600 fs-1p25 midnight">Model Management</h2>
