...
const TABLE_HEAD = [
  ...
  { id: 'designation', label: 'Designation', width: "10%", search: true, sort: true },
  { id: 'contents_html', label: 'Contents', width: "40%", search: true, sort: true },
  { id: 'image', label: 'Image', width: "15%" },
  { id: 'list_order', label: 'Order', width: "2%", sort: true, order: true },
  { id: 'status', label: 'Status', width: "8%", search: true, sort: true },
  { id: 'created', label: 'Created', width: "8%", search: true, sort: true },
...
const AdminTestimonials = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    APIHandler.adminAllTestimonials().then((data) => {
      if (isMounted) {
        setAllData(data);
        setLoading(false);
      }
  ...
  useEffect(() => {
    setTableData(allData.map((row) => ({
      ...
      image: getTestimonialImage(row.image),
  ...
  const handleDeleteRecord = () => {
    APIHandler.adminDeleteTestimonial(`(${rowData.id})`).then(data => {
      if (data.result === 'true') {
  ...
  const handleActivateRecords = (status) => {
    ...
    APIHandler.adminActiveTestimonial(`(${ids})`, status).then(data => {
      if (data.result === 'true') {
  ...
  const handleDeleteRecords = () => {
    const ids = getIdString(selected);
    APIHandler.adminDeleteTestimonial(`(${ids})`).then(data => {
      if (data.result === 'true') {
  ...
  return (
    <div className="container-fluid px-4 py-4">
      <div className="d-flex align-items-center justify-content-between">
        <h2 className="fw-600 fs-1p25 midnight">Testimonials Management</h2>
