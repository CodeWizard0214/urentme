...
const TABLE_HEAD = [
  ...
  { id: "created", label: "Created", width: "5%", search: true, sort: true },
...
const AdminPolicy = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    APIHandler.getAllPolicyList().then((data) => {
      if (isMounted) {
        setAllData(data);
        setLoading(false);
      }
  ...
  const handleDeleteRecord = () => {
    APIHandler.adminDeletePolicy(`(${rowData.id})`).then((data) => {
      if (data.result === "true") {
  ...
  const handleActivateRecords = (status) => {
    ...
    APIHandler.adminActivePolicy(`(${ids})`, status).then((data) => {
      if (data.result === "true") {
  ...
  const handleDeleteRecords = () => {
    const ids = getIdString(selected);
    APIHandler.adminDeletePolicy(`(${ids})`).then((data) => {
      if (data.result === "true") {
  ...
  return (
    <div className="container-fluid px-4 py-4">
      <div className="d-flex align-items-center justify-content-between">
        <h2 className="fw-600 fs-1p25 midnight">Privacy Policy</h2>
