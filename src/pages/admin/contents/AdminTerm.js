...
const TABLE_HEAD = [
  ...
  { id: "status", label: "Status", width: "2%", search: true, sort: true },
  { id: "created", label: "Created", width: "7%", search: true, sort: true },
...
const AdminTerm = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    APIHandler.getAllTermList().then((data) => {
      if (isMounted) {
        setAllData(data);
        setLoading(false);
      }
  ...
  const handleDeleteRecord = () => {
    APIHandler.adminDeleteTerm(`(${rowData.id})`).then((data) => {
      if (data.result === "true") {
  ...
  const handleActivateRecords = (status) => {
    ...
    APIHandler.adminActiveTerm(`(${ids})`, status).then((data) => {
      if (data.result === "true") {
  ...
  const handleDeleteRecords = () => {
    const ids = getIdString(selected);
    APIHandler.adminDeleteTerm(`(${ids})`).then((data) => {
      if (data.result === "true") {
  ...
  return (
    <div className="container-fluid px-4 py-4">
      <div className="d-flex align-items-center justify-content-between">
        <h2 className="fw-600 fs-1p25 midnight">Terms &amp; Conditions</h2>
