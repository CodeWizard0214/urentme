...
import EditDmcaModal from "./EditDmcaModal";
...
const AdminDmca = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    APIHandler.getAllDmcaList().then((data) => {
      if (isMounted) {
        setAllData(data);
        setLoading(false);
      }
  ...
  const handleDeleteRecord = () => {
    APIHandler.adminDeleteDmca(`(${rowData.id})`).then((data) => {
      if (data.result === "true") {
  ...
  const handleActivateRecords = (status) => {
    ...
    APIHandler.adminActiveDmca(`(${ids})`, status).then((data) => {
      if (data.result === "true") {
  ...
  const handleDeleteRecords = () => {
    const ids = getIdString(selected);
    APIHandler.adminDeleteDmca(`(${ids})`).then((data) => {
      if (data.result === "true") {
  ...
  return (
    <div className="container-fluid px-4 py-4">
      <div className="d-flex align-items-center justify-content-between">
        <h2 className="fw-600 fs-1p25 midnight">DMCA</h2>
