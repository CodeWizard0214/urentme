...
const EditDmcaModal = (props) => {
  ...
  const onAddData = (values) => {
    setLoading(true);
    APIHandler.adminAddDmca(values).then((data) => {
      setLoading(false);
      if (data.result === "true") {
        onSuccess();
  ...
  const onEditData = (values) => {
    ...
    APIHandler.adminEditDmca(values).then((data) => {
      setLoading(false);
      if (data.result === "true") {
        onSuccess();
