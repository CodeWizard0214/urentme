...
const TABLE_HEAD = [
  { id: "avatar", label: "Profile Image", width: "10%", search: false, sort: true },
  { id: "name", label: "Name", width: "10%", search: true, sort: true },
  { id: "designation", label: "Designation", width: "16%", search: true, sort: true },
  { id: "contents_html", label: "Contents", width: "50%", search: true, sort: true },
...
const AdminAboutUs = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    APIHandler.getAllAboutUsList().then((data) => {
      if (isMounted) {
        setAllData(data);
        setLoading(false);
      }
  ...
  useEffect(() => {
    setTableData(allData.map((row) => ({
      id: row.id,
      avatar: getMemberAvatar(row.pimage),
      name: row.name,
  ...
  const handleDeleteRecord = () => {
    APIHandler.adminDeleteAboutUs(`(${rowData.id})`).then((data) => {
      if (data.result === "true") {
  ...
  const handleActivateRecords = (status) => {
    ...
    APIHandler.adminActiveAboutUs(`(${ids})`, status).then((data) => {
      if (data.result === "true") {
  ...
  const handleDeleteRecords = () => {
    const ids = getIdString(selected);
    APIHandler.adminDeleteAboutUs(`(${ids})`).then((data) => {
      if (data.result === "true") {
  ...
  return (
    <div className="container-fluid px-4 py-4">
      <div className="d-flex align-items-center justify-content-between">
        <h2 className="fw-600 fs-1p25 midnight">About Us</h2>
