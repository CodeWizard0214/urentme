...
const TABLE_HEAD = [
  { id: "name", label: "Name", width: "11%", search: true, sort: true },
  { id: "contents_html", label: "Contents", width: "75%", search: true, sort: true },
...
const AdminPages = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    setLoading(true);
    APIHandler.getAllPagesList().then((data) => {
      if (isMounted) {
        setAllData(data);
        setLoading(false);
      }
  ...
  const handleDeleteRecord = () => {
    APIHandler.adminDeletePages(`(${rowData.id})`).then((data) => {
      if (data.result === "true") {
  ...
  const handleActivateRecords = (status) => {
    ...
    APIHandler.adminActivePages(`(${ids})`, status).then((data) => {
      if (data.result === "true") {
  ...
  const handleDeleteRecords = () => {
    const ids = getIdString(selected);
    APIHandler.adminDeletePages(`(${ids})`).then((data) => {
      if (data.result === "true") {
  ...
  return (
    <div className="container-fluid px-4 py-4">
      <div className="d-flex align-items-center justify-content-between">
        <h2 className="fw-600 fs-1p25 midnight">Pages Content</h2>
